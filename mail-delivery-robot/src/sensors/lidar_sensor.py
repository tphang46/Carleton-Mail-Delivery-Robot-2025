...
class LidarSensor(Node):
    ...
    def __init__(self):
        ...
        self.lidar_info_sub = self.create_subscription(LaserScan, "/scan", self.scan_callback,
    ...
    def calculate(self, scan):
        ...
        for i in range(count):
            ...
            # wall_following
            if degree >= self.config["WALL_FOLLOW_MIN_ANGLE"] and degree <= self.config[
                "WALL_FOLLOW_MAX_ANGLE"] and curDir < min_distance:
                min_distance = curDir
                angle = degree

            if (degree <= self.config["FRONT_MIN_ANGLE"] or degree >= self.config[
                "FRONT_MAX_ANGLE"]) and curDir < min_front:
                min_front = curDir
            elif degree >= self.config["RIGHT_MIN_ANGLE"] and degree < self.config[
                "RIGHT_MAX_ANGLE"] and curDir < min_right:
                min_right = curDir
            elif degree > self.config["LEFT_MIN_ANGLE"] and degree <= self.config[
                "LEFT_MAX_ANGLE"] and curDir < min_left:
                min_left = curDir
